/* #### Search for "bound addition" | "time addition" for any changes in the code that I made! #### */
// ===========================================================
// AST type models
...
;;         |  ( set! <var> <cexp>)            / SetExp(var: varRef, val: CExp)             #### L4
           |  ( bound? <var> )                / BoundExp(var: varRef) ####2.1.1 my addition
           |  ( time <cexp> )                 /TimeExp(exp: cexp) ####2.2.1 my addition
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | BoundExp /* bound addition */ | TimeExp /* time addition */;
...
export type SpecialFormKeyword = "lambda" | "let" | "letrec" | "if" | "set!" | "quote" | "bound?" /* bound addition */ | "time" /* time addition */;
const isSpecialFormKeyword = (x: string): x is SpecialFormKeyword =>
    ["if", "lambda", "let", "quote", "letrec", "set!", "bound?" /* bound addition */, "time" /* time addition */].includes(x);
...
export const makeBoundExp = (v: VarRef) : BoundExp => /* bound addition */
    ({tag: "BoundExp", var: v});
export const makeTimeExp = (cexp: CExp) : TimeExp => /* time addition */
...
export const isBoundExp = (x: any): x is BoundExp => x.tag === "BoundExp"; /* bound addition */
export const isTimeExp = (x: any): x is TimeExp => x.tag === "TimeExp"; /* time addition */
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    ...
    isLetrecExp(x) || isSetExp(x) || isBoundExp(x) /* bound addition */ || isTimeExp(x) /* time addition */;
...
export const parseL4SpecialForm = (op: SpecialFormKeyword, params: Sexp[]): Result<CExp> =>
    isNonEmptyList<Sexp>(params) ?
        ...
        op === "bound?" ? parseBoundExp(params) : /* bound addition */
        op === "time" ? parseTimeExp(params) : /* time addition */ 
...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> => /* bound addition */
    params.length === 1 ?
        isToken(params[0]) ? makeOk(makeBoundExp(makeVarRef(params[0] as string))) :
        makeFailure("var for bound? is a CompoundSexp = Sexp[]") :
    makeFailure("0 or more than 1 args for bound?")

const parseTimeExp = (params: Sexp[]): Result<TimeExp> => /* time addition */
    params.length !== 1 ? makeFailure("0 or more than 1 params for time") :
    bind(parseL4CExp(params[0]), (res: CExp) => makeOk(makeTimeExp(res)))
...
const unparseBoundExp = (b: BoundExp): string => /* bound addition */
        '(bound? '+b.var.var+')';

const unparseTimeExp = (t: TimeExp): string => /* time addition */
        '(time '+unparse(t.exp)+')';
...
export const unparse = (exp: Parsed): string =>
    ...
    isBoundExp(exp) ? unparseBoundExp(exp) : /* bound addition */
    isTimeExp(exp) ? unparseTimeExp(exp) : /* time addition */
