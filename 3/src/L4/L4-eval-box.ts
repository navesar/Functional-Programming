/* #### Search for "my addition" for any changes in the code that i made! #### */
// L4-eval-box.ts
...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, frameVars, isGlobalEnv, isExtEnv } from "./L4-env-box";
...
const applicativeEval = (exp: CExp, env: Env): Result<Value> =>
    ...
    isBoundExp(exp) ? evalBound(exp, env) : /* my addition */
    isTimeExp(exp) ? evalTime(exp, env) : /* my addition */
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> => /* my addition */
...
// HW3 complete this function.
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> =>{ /* my addition */
    const currTime: number = new Date().getTime();
    const val = applicativeEval(exp.exp, env);
    const updatedTime: number = new Date().getTime();
    if(isOk(val))
        return makeOk(makeCompoundSExp(val.value, updatedTime-currTime));
    return val;
}
