// ===========================================================
// AST type models
import { is, map, pipe, slice, zipWith } from "ramda";
...
export const parseCondExp = (params: Sexp[]): Result<CondExp> =>{
    if (!allT(isArray, params)) {
        ...
        if(params.length > 1 && params[params.length - 1][0] === "else" && params[params.length - 1].length > 1){
            const clausesResult = mapResult((clause: Sexp[]) =>
            safe2((test: CExp, then: CExp[]) => makeOk(makeCondClause(test, then)))
                 (parseL31CExp(first(clause)), mapResult(parseL31CExp, rest(clause))), params as Sexp[][]);
            return bind(clausesResult, (clauses: CondClause[]) => makeOk(makeCondExp(clauses)));
        }
        else{
            return makeFailure("Invalid CondExp");
        }
    }
}
...
const unparseCondClause = (clause: CondClause): string =>
    `(${unparseL31(clause.test)} ${map(unparseL31, clause.then).join(" ")})`;

const unparseCondExp = (exp: CondExp): string =>
    `(cond ${map(unparseCondClause, exp.condClauses).join(" ")})`;
