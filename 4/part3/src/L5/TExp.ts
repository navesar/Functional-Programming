...
*/
import { chain, concat, map, sort, uniq } from "ramda";
...
export type UnionTExp = {tag: "UnionTExp", comps: TExp[]};
export const makeUnionTExp = (comp1: TExp, comp2: TExp): UnionTExp => ({tag: "UnionTExp", comps: makeUnion2(comp1, comp2)});

export const makeUnion2 = (te1: TExp, te2: TExp): TExp[] =>{//helper func
    if(!isUnionTExp(te1) && !isUnionTExp(te2))
        return te1.tag === te2.tag ? [te1] :
        te1.tag < te2.tag ? [te1, te2] : [te2, te1];
    if(isUnionTExp(te1) && isUnionTExp(te2)){
        const comps1 = te1.comps;
        const comps2 = te2.comps;
        return sortAndMerge(comps1, comps2);
    }
    if(isUnionTExp(te1) && !isUnionTExp(te2))
        return sortAndMerge(te1.comps, [te2]);
    if(isUnionTExp(te2))
        return sortAndMerge([te1], te2.comps);
    return []; //never 
}
const sortAndMerge = (te1: TExp[], te2: TExp[]): TExp[] => //helper func to the helper func
    te1.length === 0 && te2.length === 0 ? [] :
    te1.length === 0 ? te2 :
    te2.length === 0 ? te1 :
    te1[0].tag === te2[0].tag ? [te1[0]].concat(sortAndMerge(te1.slice(1), te2.slice(1))) :
    te1[0].tag < te2[0].tag ? 
        te2.map((te:TExp) => te.tag).includes(te1[0].tag) ? sortAndMerge(te1.slice(1) ,te2) : 
        [te1[0]].concat(sortAndMerge(te1.slice(1), te2)) : 
    te1.map((te:TExp) => te.tag).includes(te2[0].tag) ? sortAndMerge(te1, te2.slice(1)) :
    [te2[0]].concat(sortAndMerge(te1, te2.slice(1)));
...
*/
export const parseTExp = (texp: Sexp, inUnion?: boolean): Result<TExp> =>
    ...
    isString(texp) && !inUnion ? makeOk(makeTVar(texp)) :
    isArray(texp) ? parseCompoundTExp(texp) :
    inUnion ?  makeFailure(`Unexpected TExp in Union - ${format(texp)}`) :
...
const parseCompoundTExp = (texps: Sexp[]): Result<ProcTExp | UnionTExp> => {
    const unionPos = texps.indexOf("union");
    if(unionPos === 0){
        if(texps.length !== 3)
            return makeFailure("Union exp must have two args");
        const firstTExp = parseTExp(texps[1], true);
        const secondTexp = parseTExp(texps[2], true);
        return bind(firstTExp, (val1) => bind(secondTexp, (val2) => makeOk(makeUnionTExp(val1, val2))));
    }
...
export const unparseUnion = (x: UnionTExp): Result<string> =>{
    if(x.comps.length === 0)
        return makeFailure("Error in unparsing union")
    if(x.comps.length === 1)
        return bind(unparseTExp(x.comps[0]), (arg) => makeOk(arg));
    if(x.comps.length === 2)
        return bind(unparseTExp(x.comps[0]), (arg1) => bind(unparseTExp(x.comps[1]), (arg2) => makeOk("(union "+arg1+" "+ arg2+")")));
    return bind(unparseTExp(x.comps[0]), (val) => bind(unparseTExp(makeUnionTExp2(x.comps.slice(1))), (ans) => makeOk("(union "+ val+" "+ans+")")));
}
export const makeUnionTExp2 = (tes: TExp[]): UnionTExp =>
    ({tag: "UnionTExp", comps: tes});
